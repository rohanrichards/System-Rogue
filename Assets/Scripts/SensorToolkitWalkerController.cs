...
namespace CMF
{
	...
	public class SensorToolkitWalkerController : Controller {
		...
		public RangeSensor visionSensor;
		...
		void Awake () {
			...
			steeringRig = GetComponentInChildren<SteeringRig>();
			visionSensor = GetComponentInChildren<RangeSensor>();
		...
		{
			Vector3 _velocity = Vector3.zero;
			List<GameObject> visibleObjects = visionSensor.GetDetected();
			if(visibleObjects.Count > 0)
            {
				GameObject target = visibleObjects[0];
				Vector3 targetDirection = (target.transform.position - transform.position).normalized;
				//Debug.Log("target Object: " + target + ", Target direction: " + targetDirection);
				_velocity += steeringRig.GetSteeredDirection(targetDirection);
			}
